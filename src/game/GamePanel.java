...
public class GamePanel extends JPanel implements Runnable{ // because GamePanel already extend JPanel so must use Runnable 
	...
	public void run() {
		...
		while(running) { // better loop //update more times than render -> react fast
			...
			int updateCount = 0;
			while((now - lastUpdateTime > TBU) && (updateCount < MUBR)) {// update 5 times per frame if possible
				update();
				input(mouse);
				lastUpdateTime += TBU;
				updateCount++;
			...
			while(now - lastRenderTime < TTBR && now - lastUpdateTime < TBU) {// make sure the fps and hz stay at 60
				Thread.yield();
	...
	public void render() {
		if(g != null) {
			...
			g.fillRect(0, 0, width, height*3/5);
			g.setColor(new Color(23, 100, 244));
			g.fillRect(0, height*3/5, width, height);
			g.setColor(new Color(200, 100, 50));
			g.fillRect(0, height*3/5, width, height/12);
			gsm.render(g);
		}
	}
